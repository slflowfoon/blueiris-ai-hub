...
class TestRunMonitorLoop:
    def test_processes_queued_request(self, monkeypatch):
        """run_monitor must pop and process a queued request."""
        _push_request()
        processed = []
...
class TestPreResolvedClip:
    """#45 -- monitor must use pre-resolved clip_path from payload."""
    ...
    def test_pre_resolved_clip_skips_alertlist(self, monkeypatch):
        ...
        # Initial export response must contain 'path' and 'uri'
        ...
        # UPDATED: Use new queue completion function
...
class TestPersistent404FastFail:
    """#46 -- 50+ consecutive 404s must break out early."""
